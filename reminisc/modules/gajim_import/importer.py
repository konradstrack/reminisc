...
# In sqlite3, autoincrement primary index is guaranteed to be monotinically increasing
# and that the selected row id has never been used in the same table before.
# This allows us to sort by log_line_id instead of time, which is siginificantly faster.
#
...
class IterativeImporter(object):
	...
	def start(self):
		...
		polling_interval = float(self.module_config.get('gajim', 'polling_interval', fallback=1))
